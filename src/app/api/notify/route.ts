...
/**
 * POST /api/notify — sends the admin notification email via the
 * info@crownlegacydobermans.com mailbox over Hostinger SMTP. The mailbox
 * password stays server-only (never NEXT_PUBLIC_). Always resolves 200, so
 * a missing password or an SMTP failure never blocks the caller's form.
 */
export async function POST(request: Request) {
  const pass = process.env.SMTP_PASS;
  if (!pass) return NextResponse.json({ ok: false });
  ...
  try {
    ...
    // instead of the site's own mailbox.
    const replyTo = typeof payload.email === 'string' && payload.email.trim() ? payload.email : undefined;

    // The signature pad hands us a `data:image/png;base64,...` URL —
    // nodemailer attachments want just the base64 payload.
    const signature = typeof payload.signature === 'string' ? payload.signature : '';
    const attachments =
      payload.type === 'contract' && signature.startsWith('data:image/png;base64,')
        ? [{ filename: 'signature.png', content: signature.split(',')[1], encoding: 'base64' as const }]
        : undefined;
