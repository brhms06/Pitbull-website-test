...
export default function PuppyApplicationForm({ dogId, dogName, dogs }: Props) {
  ...
  const update = (key: keyof ApplicationData, value: string | boolean) => {
  ...
  const validateStep = (current: number): boolean => {
    ...
    if (current === 2 && !data.agree) {
      next.agree = 'Please confirm you accept the reservation process.';
    }
    setErrors(next);
      ...
      <form onSubmit={onSubmit} noValidate className="card p-6 sm:p-8">
        <ol className="mb-6 flex items-center gap-2">
          {steps.map((label, i) => (
            <li key={label} className="flex flex-1 items-center gap-2">
              ...
              <span className={`hidden text-xs font-semibold sm:block ${i <= step ? 'text-forest-800' : 'text-muted'}`}>{label}</span>
              {i < steps.length - 1 && <span className="mx-1 hidden h-0.5 flex-1 rounded bg-sand sm:block" />}
            </li>
          ))}
        </ol>
        ...
        <AnimatePresence mode="wait">
          <motion.div key={step} initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -24 }} transition={{ duration: 0.25 }}>
            ...
            {step === 1 && (
              <div className="grid gap-5 sm:grid-cols-2">
                <FormField label="City & ZIP code area" htmlFor="a-addr" required error={errors.address} className="sm:col-span-2">
                  <input id="a-addr" className={inputCls('address')} value={data.address} onChange={(e) => update('address', e.target.value)} placeholder="e.g. Denver, CO 80202" />
                </FormField>
                <FormField label="Home type" htmlFor="a-home">
                  <select id="a-home" className="input" value={data.homeType} onChange={(e) => update('homeType', e.target.value)}>
                ...
                <FormField label="Children at home?" htmlFor="a-children">
                  <select id="a-children" className="input" value={data.hasChildren} onChange={(e) => update('hasChildren', e.target.value)}>
                    <option>No</option>
                ...
                <FormField label="Other pets?" htmlFor="a-pets" className="sm:col-span-2">
                  <select id="a-pets" className="input" value={data.hasPets} onChange={(e) => update('hasPets', e.target.value)}>
                    <option>No</option>
              ...
              <div>
                <div className="rounded-2xl bg-forest-50 p-5 text-sm">
                  ...
                  <ul className="mt-3 space-y-1.5 text-ink/80">
                    ...
                    <li>
                      <strong>Home:</strong> {data.homeType}
                    </li>
                  </ul>
                </div>
                ...
                <label className="mt-4 flex items-start gap-3 text-sm">
                  ...
                  <span>I understand and accept the reservation process described above.</span>
                </label>
                {errors.agree && (
        ...
        <div className="mt-7 flex items-center justify-between gap-3">
          <button type="button" onClick={goBack} disabled={step === 0} className="btn-ghost disabled:invisible">
            Back
          </button>
          {step < steps.length - 1 ? (
            ...
            <motion.button type="submit" whileTap={{ scale: 0.98 }} disabled={submitting} className="btn-accent disabled:opacity-70">
