  ...
  if (count === 0) {
    return (
      <div className="container-page flex min-h-[60vh] flex-col items-center justify-center gap-4 py-16 text-center">
        ...
        <p className="max-w-md text-muted">Browse our available puppies and reserve your new companion.</p>
        <Link href="/dogs" className="btn-primary">
          Browse puppies <ArrowRightIcon className="h-5 w-5" />
        </Link>
      </div>
    );
  }
  ...
  return (
    <div className="container-page py-12 md:py-16">
      ...
      <p className="mt-1 text-muted">
        {count} {count === 1 ? 'puppy' : 'puppies'} reserved for you
      </p>
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_360px]">
        <div className="space-y-4">
          {items.map((item) => (
            <div key={item.id} className="card flex items-center gap-4 p-4">
              ...
              <button type="button" onClick={() => remove(item.id)} aria-label={`Remove ${item.name}`} className="shrink-0 rounded-full p-2 text-muted transition hover:bg-red-50 hover:text-red-600">
                <TrashIcon className="h-5 w-5" />
              </button>
            </div>
          ))}
          <Link href="/dogs" className="inline-flex items-center gap-1 text-sm font-semibold text-forest-600 hover:underline">
            ← Continue browsing
          </Link>
        </div>
        ...
        <aside className="lg:sticky lg:top-24 lg:self-start">
          <div className="card p-6">
            ...
            <Link href="/checkout" className="btn-accent mt-5 w-full text-base">
